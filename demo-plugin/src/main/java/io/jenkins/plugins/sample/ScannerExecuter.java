...
public class ScannerExecuter {
  ...
  public static int execute(Run<?, ?> build, FilePath workspace, Launcher launcher, TaskListener listener, String artifactName,
      Secret microScannerToken, String imageName, String notCompliesCmd, boolean checkonly, boolean caCertificates){
    ...
    try {
      ...
      microScannerDockerfileContent.append("RUN chmod +x microscanner && ./microscanner ${token} ").append("--html ");
      if (checkonly)
      {
      ...
      FilePath latestTarget = new FilePath(workspace, "scanlatest." + "html");
      ...
      if (exitCode == PortshiftVulnerabilityScannerBuilder.DISALLOWED_CODE && !notCompliesCmd.trim().isEmpty()) {
        ps = launcher.launch();
